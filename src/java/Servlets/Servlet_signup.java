 ...
 * @author david
 */
@WebServlet(name = "Servlet_signup", urlPatterns = {"/Servlet_signup"})
public class Servlet_signup extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Servlet_signup</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Servlet_signup at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            gesUser.crearUsuario(user, passwd);
        } catch (Exception ex) {
            Logger.getLogger(Servlet_registro.class.getName()).log(Level.SEVERE, null, ex);
        } finally{
            response.sendRedirect("ComponenteLogin.jsp");
        }
